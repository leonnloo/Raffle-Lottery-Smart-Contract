...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          ...
          beforeEach(async () => {
              ...
              const raffleCon = await deployments.get("Raffle") // Returns a new connection to the Raffle contract
              raffleContract = await ethers.getContractAt(raffleCon.abi, raffleCon.address)
              ...
              it("picks a winner, resets, and sends money", async () => {
                  ...
                  await new Promise(async (resolve, reject) => {
                      ...
                      try {
                          ...
                          console.log("PerformUpkeep called, waiting for receipt")
                          const txReceipt = await tx.wait(1)
                          console.log("Transaction receipt received")
                          startingBalance = await ethers.provider.getBalance(
                              accounts[2].getAddress(),
                          )
                          console.log("Starting balance fetched")
                          ...
                          console.error("Error in performUpkeep block:", e)
                          reject(e)
                      }
                  })
              })
